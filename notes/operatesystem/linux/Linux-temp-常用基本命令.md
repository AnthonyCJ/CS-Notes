...
* **绝对路径：**
  * 路径的写法，由根目录 / 写起，例如：要访问 /usr/share/doc 这个目录，则使用命令 `cd /usr/share/doc `
* **相对路径：**
  * 路径的写法，不是由 / 写起，例如由当前目录 /usr/share/doc 要到 /usr/share/man 使用命令 `cd ../man` 这就是相对路径的写法啦！
...
- `-a` ：全部的文件，包括隐藏文件( 开头为 . 的文件) 一起列出来(常用)
...
# 回到当前的用户目录，亦即是 /root 这个目录
cd ~
```

接下来大家多操作几次应该就可以很好的理解 cd 命令的。

#### 1.2.3 `pwd` ( 显示目前所在的目录路径 )
...
- `nl`  显示文件内容，同时<font color="ff9f44">输出行号</font>
- `more` 一页一页的显示文件内容
...
- Space（空格键）：<font color="ff9f44">向下翻一页</font>；
- Enter （回车键）  ：<font color="ff9f44">向下翻一行</font>；
...
#### 3.2.5 `less`  一页一页翻动，以下实例输出/etc/man.config文件的内容：
...
- 同时删除原文件f1,硬连接f2，整个文件会真正的被删除。显示